...
`),
	...
	if err.Error() != "validation: no Chart.yaml data was found in this chart" {
...
func TestLoadInvalidArchive(t *testing.T) {
	...
	for _, tt := range []struct {
		...
		{"illegal-name4.tgz", "/missing-leading-slash", "validation: no Chart.yaml data was found in this chart"},
