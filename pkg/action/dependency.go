	...
	case len(archives) > 1:
		...
		for _, arc := range archives {
			...
			if len(parts) < 2 {
				...
				panic("string matched glob for '-' but does not contain any '-'")
			}
			println(parts[1])
			if _, err := semver.StrictNewVersion(parts[1]); err == nil {
		...
		if l := len(found); l == 1 {
			...
			println("Falling thru because no matches found")
		} else if l > 1 {
